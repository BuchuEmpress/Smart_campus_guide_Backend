...
async def get_cache_status():
    try:
        ...
        message = "Cache is up to date" if not is_stale else f"Cache is stale (last updated {time_since_update.days} days ago)"
...
async def generate_cache(
    ...
    """
    Generate or refresh offline cache asynchronously.
    """
    try:
        services = get_services()
        ...
        # ✅ Add async-safe wrapper for background task
...
async def _generate_cache_task(
    ...
    """
    Background task for cache generation. Async now.
    """
    try:
        logger.info("Executing cache generation task")

        # ✅ Await async method
        popular_locations = await analytics_service.get_popular_locations(
            min_searches=min_searches,
            limit=limit
        )

        success = cache_service.generate_cache(popular_locations=popular_locations)
...
async def get_popular_locations(
    ...
    try:
        ...
        popular = await services["analytics"].get_popular_locations(
            min_searches=min_searches,
            limit=limit
        )
